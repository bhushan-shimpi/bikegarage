...
export const Modal: React.FC<ModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`relative w-full ${maxWidthClasses[maxWidth]} bg-[#151515] border border-[#262626] rounded-xl shadow-2xl overflow-hidden my-8`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#262626] bg-[#111111]">
          {title ? (
            <h3 id="modal-title" className="text-lg font-bold text-white tracking-wide uppercase">
              {title}
            </h3>
          ) : (
            <div />
