...
export const AdminPartsInventoryPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex items-start sm:items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 flex items-center gap-2.5">
            ...
            <span className="truncate">Spare Parts Price List</span>
          </h1>
      ...
      {/* ─── PARTS LIST: SEPARATE CARD FOR EACH ROW ─── */}
      {loading ? (
        <div className="bg-white rounded-2xl border border-gray-200 py-12 text-center text-xs text-gray-400 shadow-xs">
          Loading spare parts price list...
        </div>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-200 py-12 text-center space-y-2 shadow-xs">
        ...
        <div className="space-y-2.5">
          {filtered.map((part) => (
            <div
              key={part.id}
              className="bg-white rounded-2xl border border-gray-200 hover:border-amber-300 p-3.5 sm:p-4 shadow-xs transition-all flex items-center justify-between gap-3"
            >
              {/* Left: Icon, Part Name & Category Badge */}
              <div className="min-w-0 flex-1 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <span className="px-2 py-0.2 rounded text-[10px] font-bold uppercase tracking-wider bg-gray-100 text-gray-600 border border-gray-200 inline-block mt-0.5">
                    {part.category}
                  </span>
                </div>
              </div>

              {/* Center: Inline Price Editing */}
                ...
                {inlineEditId === part.id ? (
                  <div className="flex items-center gap-1.5">
                    ...
                    <input
                      ...
                      className="w-20 sm:w-24 bg-white border-2 border-emerald-500 rounded-lg px-2 py-1 text-xs font-mono font-bold text-gray-900 focus:outline-none"
                      autoFocus
                    />
                    <button
                      type="button"
                  ...
                  <button
                    ...
                    className="group/price flex items-center gap-1.5 font-mono font-black text-sm text-emerald-800 hover:text-emerald-950 bg-emerald-50 hover:bg-emerald-100 px-2.5 py-1.5 rounded-xl border border-emerald-200 transition-colors"
              ...
              {/* Right: Full Edit & Delete buttons */}
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  type="button"
                  onClick={() => handleOpenEdit(part)}
                  className="p-2 rounded-xl border border-gray-200 bg-gray-50 text-gray-700 hover:bg-[#F5B900] hover:text-black transition-colors"
